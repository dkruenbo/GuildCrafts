...
local SYNC_CHUNK_DELAY     = 1.0     -- seconds between sync chunks (avoids burst lag)
...
function Comms:OnInitialize()
    ...
    -- Pending re-sync debounce timer
...
function Comms:HandleHello(payload, sender)
    ...
    -- Debounced: multiple rapid discoveries collapse into a single sync
    -- 10s after the last one, avoiding N simultaneous re-sync bursts.
...
--- Chunks are spaced SYNC_CHUNK_DELAY seconds apart to avoid burst lag
--- on large guilds (contributed by PR #72).
